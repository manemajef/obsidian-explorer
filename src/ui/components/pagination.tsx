		...
		<div dir="ltr" style={{ display: "flex", justifyContent: "center" }}>
			<div className="paging-controll">
				...
				<div className="paging-controll-nums">
					{page !== 0 ? (
						<span className="paging-num paging-label" onClick={() => onPageChange(0)}>
							1
						</span>
					) : null}
					...
					{leftPages.map((p) => (
						<span key={p} className="paging-label paging-num" onClick={() => onPageChange(p)}>
							{p + 1}
						</span>
					))}

					<span className="paging-label active-page paging-num">{page + 1}</span>
					...
					{page !== totalPages - 1 ? (
						<span className="paging-num paging-label" onClick={() => onPageChange(totalPages - 1)}>
							{totalPages}
						</span>
					) : null}
				</div>
